...
def ransac_plane_fit(xyz_data):
    ...
    min_num_inliers = 1000  # RANSAC minimum number of inliers
    ...
    for i in range(num_itr):
        ...
        random_pts_index = np.random.choice(xyz_data.shape[1], 3)
        #print(random_pts_index)
        random_pts = xyz_data[:, random_pts_index]
        # Step 2: Compute plane model
        plane = compute_plane(random_pts)
        # Step 3: Find number of inliers
        plane_r = plane.reshape((4, 1))
        x_r = xyz_data[0, :].reshape((xyz_data.shape[1], 1))
        y_r = xyz_data[1, :].reshape((xyz_data.shape[1], 1))
        z_r = xyz_data[2, :].reshape((xyz_data.shape[1], 1))
        dist = np.abs(dist_to_plane(plane_r, x_r, y_r, z_r))
        num_inliers = 0
        for x in dist:
        ...
        if num_inliers > min_num_inliers:
            x_keep = x_r[dist < distance_threshold]
            y_keep = y_r[dist < distance_threshold]
            z_keep = z_r[dist < distance_threshold]
            xyz_inlier_set = np.stack((x_keep, y_keep, z_keep))
        # Step 5: Check if stopping criterion is satisfied and break.         

        
    # Step 6: Recompute the model parameters using largest inlier set.         
    output_plane = compute_plane(xyz_inlier_set)
    ### END CODE HERE ###
...
def estimate_lane_lines(segmentation_output):
    ...
    image_mask = np.zeros((segmentation_output.shape)).astype('uint8')
    
    image_mask[segmentation_output == 6] = 255
    image_mask[segmentation_output == 8] = 255

    # Step 2: Perform Edge Detection using cv2.Canny()
    edge = cv2.Canny(image_mask, 150, 200)

    # Step 3: Perform Line estimation using cv2.HoughLinesP()
    lines = cv2.HoughLinesP(edge, 5, np.pi/180, 200, minLineLength = 100, maxLineGap = 30)
...
def filter_detections_by_segmentation(detections, segmentation_output):
    ...
    for detection in detections:
        ...
        if Class == 'Car':
            ...
            Class_index = 4
        pixels = len(np.where(segmentation_output[y_min:y_max, x_min:x_max] == Class_index)[0])
        # Step 2: Devide the computed number of pixels by the area of the bounding box (total number of pixels).
        ratio = pixels / bbox_area    
...
def find_min_distance_to_detection(detections, x, y, z):
    ...
    for detection in detections:
        ...
        #print(x_box)
        dist = np.sqrt(x_box**2 + y_box**2 + z_box**2)
